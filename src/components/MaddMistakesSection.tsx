...
import { faCheck, faArrowsLeftRight } from '@fortawesome/free-solid-svg-icons';
import { useAssessment } from '../contexts/AssessmentContext';

const MaddMistakesSection: React.FC = () => {
  const { 
    selectedSurah, 
    checklistMistakes, 
  ...
  const maddMistakes = [
    {
      key: 'incorrectElongation',
      title: 'Incorrect elongation (shortened)',
      description: 'Madd not held for proper duration'
    },
    {
      key: 'overElongation',
      title: 'Over-elongation',
      description: 'Madd held longer than required'
    },
    {
      key: 'wrongMaddType',
      title: 'Wrong Madd type',
      description: 'Asli, Muttasil, Munfasil confusion'
    },
    {
      key: 'inconsistentLength',
      title: 'Inconsistent Madd lengths',
      description: 'Varying durations for same Madd type'
    },
  ];
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center', color: '#6b7280' }}>
          <FontAwesomeIcon icon={faArrowsLeftRight} style={{ fontSize: '2rem', marginBottom: '0.5rem' }} />
          <div style={{ fontWeight: '500' }}>Madd Mistakes</div>
  ...
  const deduction = calculateMaddDeduction();
      ...
      <div style={{ padding: '1.5rem' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <FontAwesomeIcon icon={faArrowsLeftRight} style={{ fontSize: '1.5rem', color: '#0ea5e9' }} />
            <div>
              <h3 style={{
                fontSize: '1.125rem',
                fontWeight: '600',
              ...
              <p style={{
                ...
                color: '#6b7280',
                margin: 0
              }}>Elongation errors (2.5% each, max 10%)</p>
            </div>
          </div>
        ...
        <div style={{
          ...
          {maddMistakes.map((mistake) => {
            const isChecked = checklistMistakes.maddMistakes[mistake.key as keyof typeof checklistMistakes.maddMistakes];
            
            return (
              <div
                key={mistake.key}
                onClick={() => toggleChecklistMistake('maddMistakes', mistake.key)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '0.75rem',
...
export default MaddMistakesSection;
